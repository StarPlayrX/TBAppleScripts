-- DRAW DIELINE v1.10.CS
-- script by todd bruss ©Ê2005 - 2020

-- v1.10.2
-- shortened up code, introduced subroutines

-- Requires Adobe Illustrator CS 11.0.0
...
set x to display dialog "DRAW DIELINE CS v1.10.3" & return & return & "die measurements:" buttons {"millimeters", "inches"} default button doc_units with icon 1
...
tell application "Adobe Illustrator"
	...
	if doc_units = "inches" then
		set textRef to make new Çclass cTXaÈ in document 1 Â
			with properties {Çclass pCNTÈ:("CUTOFF " & label_h as text) & unit_label, Çclass paPsÈ:{(-72 / 2 - x * 2.5), (b1 / 2 - x)}}
		set rotationMatrix to Çevent ARTxgeRMÈ given Çclass pAGLÈ:-90
		Çevent ARTxTRANÈ textRef given Çclass TRmxÈ:rotationMatrix, Çclass pTXxÈ:Çconstant eTBae122È
	else
		set textRef to make new Çclass cTXaÈ in document 1 Â
			with properties {Çclass pCNTÈ:("CUTOFF " & label_h as text) & unit_label, Çclass paPsÈ:{(-72 / 2 - x * 3.5), (b1 / 2 - x)}}
	...
	if ((item 1 of label_w as number) * 72 / unit_maker) > 10 then
		set textRef2 to make new Çclass cTXaÈ in document 1 Â
			with properties {Çclass pCNTÈ:(item 1 of label_w as text) & unit_label, Çclass paPsÈ:{(c1 / 2), (-72 / 2) + 14}}
	else
		set textRef2 to make new Çclass cTXaÈ in document 1 Â
			with properties {Çclass pCNTÈ:(item 1 of label_w as text) & unit_label, Çclass paPsÈ:{(c1 / 2), (-72 / 2) + 7}}
	end if
...
repeat with i from 2 to the_count
	...
	tell application "Adobe Illustrator"
		...
		if ((item i of label_w as number) * 72 / unit_maker) > 10 then
			set textRef2 to make new Çclass cTXaÈ in document 1 Â
				with properties {Çclass pCNTÈ:((item i of label_w) as text) & unit_label, Çclass paPsÈ:{(c1 - (((item i of label_w as number) * 72 / unit_maker) / 2)), (-72 / 2) + 14}}
			
		else
			set textRef2 to make new Çclass cTXaÈ in document 1 Â
				with properties {Çclass pCNTÈ:((item i of label_w) as text) & unit_label, Çclass paPsÈ:{(c1 - (((item i of label_w as number) * 72 / unit_maker) / 2)), (-72 / 2) + (shortie)}}
		end if
	end tell
...
if skip_me = "Skip" then
	...
	tell application "Adobe Illustrator"
		set textRef2 to make new Çclass cTXaÈ in document 1 Â
			with properties {Çclass pCNTÈ:("LAYFLAT " & (doc_half) as text) & unit_label as text, Çclass paPsÈ:{(c1 / 4), -50}}
		
		set textRef2 to make new Çclass cTXaÈ in document 1 Â
			with properties {Çclass pCNTÈ:("LAYFLAT " & (doc_half) as text) & unit_label as text, Çclass paPsÈ:{(c1 / 2 + c1 / 4), -50}}
	end tell
...
tell application "Adobe Illustrator"
	-- web dim amount
	set textRef2 to make new Çclass cTXaÈ in document 1 Â
		with properties {Çclass pCNTÈ:(("WEB " & doc_web) as text) & unit_label, Çclass paPsÈ:{(c1 / 2), 36 + b1}}
	-- try
	...
	tell document 1
		...
		set Çclass pT16È of every paragraph of every Çclass cTXaÈ to {Çconstant eTBae122È}
